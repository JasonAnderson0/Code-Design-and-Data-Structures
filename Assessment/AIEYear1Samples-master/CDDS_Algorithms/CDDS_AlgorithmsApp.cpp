...
bool CDDS_AlgorithmsApp::checkForWin()
{
	...
	for (int i = 0; i < COLUMNS; i++) {
		for (int j = 0; j < ROWS; j++) {
			int tile = m_board[i][j];
			if (tile == 0)
				continue;
			//check horizontal
			if (tile == m_board[i][j] && tile == m_board[i][j + 1] && tile == m_board[i][j + 2] && tile == m_board[i][j + 3]) {
				return true;
			}

			//check vertical
			if (tile == m_board[i][j] && tile == m_board[i + 1][j] && tile == m_board[i + 2][j] && tile == m_board[i + 3][j]) {
				return true;
			}

			//check 
			if (tile == m_board[i][j] && tile == m_board[i + 1][j + 1] && tile == m_board[i + 2][j + 2] && tile == m_board[i + 3][j + 3]) {
				return true;
			}
